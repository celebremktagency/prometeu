--
-- PROMETEUS - SCRIPT DE RESTAURAÇÃO COMPLETA
-- Extraído do backup: db_cluster-03-10-2025@03-14-29.backup
-- Total: 6.362 linhas originais, filtrado para restauração
--
...
-- Criar esquemas se não existirem (Supabase já tem, mas garantimos)
-- CREATE SCHEMA IF NOT EXISTS "auth";
-- CREATE SCHEMA IF NOT EXISTS "storage";
-- CREATE SCHEMA IF NOT EXISTS "realtime";

-- ===========================================
-- TYPES E ENUMS CUSTOMIZADOS
-- ===========================================

-- Tipo para factor types (MFA)
DO $$ BEGIN
...
-- Tipo para code challenge method
DO $$ BEGIN
...
-- ===========================================
-- TABELAS PRINCIPAIS DO PROMETEUS
-- ===========================================
...
CREATE TABLE IF NOT EXISTS public.treinos_atribuidos (
    ...
    CONSTRAINT treinos_atribuidos_treino_id_fkey FOREIGN KEY (treino_id) REFERENCES public.treinos(id) ON DELETE CASCADE
);
...
DROP POLICY IF EXISTS "Users can only see their own workout logs" ON public.treinos_logs;
CREATE POLICY "Users can only see their own workout logs" ON public.treinos_logs
  FOR ALL USING (auth.uid() = usuario_id);

DROP POLICY IF EXISTS "Users can only see their own pain logs" ON public.dores_logs;
CREATE POLICY "Users can only see their own pain logs" ON public.dores_logs
  FOR ALL USING (auth.uid() = usuario_id);

DROP POLICY IF EXISTS "Users can only see their own streaks" ON public.user_streaks;
CREATE POLICY "Users can only see their own streaks" ON public.user_streaks
  FOR ALL USING (auth.uid() = user_id);

-- Políticas para personal-aluno relacionamento
DROP POLICY IF EXISTS "Personal can see their students" ON public.personal_aluno;
CREATE POLICY "Personal can see their students" ON public.personal_aluno
  FOR ALL USING (auth.uid() = personal_id OR auth.uid() = aluno_id);

-- Treinos são públicos para leitura
DROP POLICY IF EXISTS "Treinos are public for reading" ON public.treinos;
CREATE POLICY "Treinos are public for reading" ON public.treinos
  FOR SELECT USING (true);

-- Comunidade é pública para leitura
...
DROP POLICY IF EXISTS "Users can create their own posts" ON public.comunidade_posts;
CREATE POLICY "Users can create their own posts" ON public.comunidade_posts
  FOR INSERT WITH CHECK (auth.uid() = usuario_id);

DROP POLICY IF EXISTS "Users can update their own posts" ON public.comunidade_posts;
CREATE POLICY "Users can update their own posts" ON public.comunidade_posts
  FOR UPDATE USING (auth.uid() = usuario_id);

-- Comentários são públicos para leitura
DROP POLICY IF EXISTS "Comments are public" ON public.comunidade_comentarios;
CREATE POLICY "Comments are public" ON public.comunidade_comentarios
  FOR SELECT USING (true);

DROP POLICY IF EXISTS "Users can create comments" ON public.comunidade_comentarios;
CREATE POLICY "Users can create comments" ON public.comunidade_comentarios
  FOR INSERT WITH CHECK (auth.uid() = usuario_id);

-- Curtidas
DROP POLICY IF EXISTS "Users can manage their own likes" ON public.comunidade_curtidas;
CREATE POLICY "Users can manage their own likes" ON public.comunidade_curtidas
  FOR ALL USING (auth.uid() = usuario_id);

-- Workout templates são públicos
